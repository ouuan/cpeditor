 ...
 */
Diff::Diff(Operation _operation, const QString &_text) : operation(_operation), text(_text)
{
...
diff_match_patch::diff_match_patch()
    : Diff_Timeout(1.0f), Diff_EditCost(4), Match_Threshold(0.5f), Match_Distance(1000), Patch_DeleteThreshold(0.5f),
      Patch_Margin(4), Match_MaxBits(32)
{
}
...
QList<Diff> diff_match_patch::diff_bisectSplit(const QString &text1, const QString &text2, int x, int y,
                                               clock_t deadline)
{
    ...
    while (nextDiff != NULL)
    {
        if (prevDiff->operation == EQUAL && nextDiff->operation == EQUAL)
        {
            ...
            if (thisDiff->text.endsWith(prevDiff->text))
            {
                ...
                thisDiff->text =
                    prevDiff->text + thisDiff->text.left(thisDiff->text.length() - prevDiff->text.length());
...
void diff_match_patch::patch_addContext(Patch &patch, const QString &text)
{
    ...
    // Add the prefix.
    QString prefix =
        safeMid(text, std::max(0, patch.start2 - padding), patch.start2 - std::max(0, patch.start2 - padding));
    if (!prefix.isEmpty())
    {
    ...
    QString suffix =
        safeMid(text, patch.start2 + patch.length1,
                std::min(text.length(), patch.start2 + patch.length1 + padding) - (patch.start2 + patch.length1));
    if (!suffix.isEmpty())
    {
...
QList<Patch> diff_match_patch::patch_make(const QString &text1, const QList<Diff> &diffs)
{
    ...
    foreach (Diff aDiff, diffs)
    {
        ...
        case DELETE:
            ...
            patch.diffs.append(aDiff);
            postpatch_text =
                postpatch_text.left(char_count2) + safeMid(postpatch_text, char_count2 + aDiff.text.length());
            break;
        case EQUAL:
    ...
    foreach (Patch aPatch, patchesCopy)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if (text1.length() > Match_MaxBits &&
                    diff_levenshtein(diffs) / static_cast<float>(text1.length()) > Patch_DeleteThreshold)
                {
                        ...
                        if (aDiff.operation != EQUAL)
                        {
                            ...
                            else if (aDiff.operation == DELETE)
                            {
                                ...
                                text = text.left(start_loc + index2) +
                                       safeMid(text, start_loc + diff_xIndex(diffs, index1 + aDiff.text.length()));
                            }
                        }
                        if (aDiff.operation != DELETE)
                        {
...
QString diff_match_patch::patch_addPadding(QList<Patch> &patches)
{
    ...
    else if (paddingLength > firstPatchDiffs.first().text.length())
    {
        ...
        firstDiff.text =
            safeMid(nullPadding, firstDiff.text.length(), paddingLength - firstDiff.text.length()) + firstDiff.text;
